...
Distributions of champion diffs don't point to anything useful. Boxplots
...
| **Normality**               | `_diff` features are roughly symmetric, `kill_diff` somewhat resembles normal. `first_*` are categorical. `_champdiff` are discrete, centered at 0. Not strictly required for tree-based models.                                                                                                                                      |
| **Feature scaling**         | Ranges of `_diff` and `_champdiff` features are relatively small. Scaling is not required for decision trees but might be for other models.                                                                                                                                                                                           |
| **Homoscedasticity**        | `_diff` and `_champdiff` features generally show similar variance across `team1_won` groups (homoscedasticity mostly holds for numeric predictors). Not a strict requirement for decision trees.                                                                                                                                      |
| **Outliers**                | EDA didn't reveal extreme, influential outliers in numeric features that would require special handling for decision trees.                                                                                                                                                                                                           |
...
tree based methods can potentially capture any complex interactions if present
and inherently perform feature selection.
