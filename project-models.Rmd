...
confusionMatrix(data = predictions$predictions, reference = test_data$team1_won, positive = "1")
```

This model gave more balanced predictions, makes roughly an equal number of FPs
and FNs. But the accuracy is very similar to the best tuned decision tree.
Let's see what happens when we let more features into the model.
...
accuracy, but worse balance, then the random forest. We are stopping trying
...
model, we now compare their performance on the test set. Our goal is to
understand which model performed best and how the different modeling approaches
handled the prediction task based on our selected early-game features.

We focus on key classification metrics:
...
Here is a summary table of the test set performance for our best representatives
of each model type:
...
1.  **Accuracy:** All three models achieved similar overall accuracy,
	falling within a narrow range of approximately 67% to 68.1%.
...
2.  **Balance:** This is where the models show clearer differences:
    ...
    *   The **AdaBoost.M1** model sits in between the other two in terms of
	    balance. It has higher Sensitivity (72.5%) than the Random Forest, but
...
the best model from those tested. While its overall accuracy is only marginally
...
given features. Also, games of League of Legends can also be won after early
game, early game state doesn't have dictate the end game result.
...
*   The initial **Decision Tree** showed high Sensitivity (good at predicting wins)
	...
	the positive class.
*   The **Random Forest**, particularly with the full feature set, achieved the
