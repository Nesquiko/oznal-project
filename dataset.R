...
event_counts_by_type <- function(
  ...
  events %>%
    ...
    left_join(
      player_team_lookup,
      by = c("game_id", "killer_id" = "player_id")
    ) %>%
...
find_first_event <- function(
  ...
  event_data %>%
    ...
    left_join(
      event_data %>%
        filter(event_type == event_type_name) %>%
        select(game_id, timestamp, killer_id),
    ...
    left_join(
      games_data %>% select(game_id, team1_id, team2_id),
      by = "game_id"
    ) %>%
    mutate(
      first_event_team = case_when(
...
early_game_dataset <- function(
  ...
  first_dragons <- find_first_event(
    events,
    player_team_lookup,
    games,
    "drake_kill"
  ...
  first_heralds <- find_first_event(
    events,
    player_team_lookup,
    games,
    "rift_herald_kill"
